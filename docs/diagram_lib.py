...
def draw_title(ax, x, y, text, subtitle=None):
    ...
    if subtitle:
        ax.text(x, y - GRID * 1.3, subtitle, fontsize=10.5, color=COLOR_TEXT_MUTED, ha="left", va="top", **FONT_BODY)
...
def draw_card(ax, x, y, w, h, label, sublabel=None, accent=False, muted=False):
    ...
    if sublabel:
        ax.text(x + w / 2, y + h * 0.60, label, fontsize=10, color=text_color, ha="center", va="center", **FONT_TITLE)
        ax.text(
            x + w / 2, y + h * 0.30, sublabel, fontsize=8, color=COLOR_TEXT_MUTED, ha="center", va="center", **FONT_BODY
        )
    else:
        ax.text(x + w / 2, y + h / 2, label, fontsize=10, color=text_color, ha="center", va="center", **FONT_TITLE)
...
def draw_database(ax, x, y, w, h, label, engine=None):
    ...
    cap_h = h * 0.16
    body = Rectangle((x, y + cap_h / 2), w, h - cap_h, linewidth=1.1, edgecolor=COLOR_BORDER_STRONG, facecolor=COLOR_BG)
    ax.add_patch(body)
    bottom_cap = Ellipse(
        (x + w / 2, y + cap_h / 2), w, cap_h, linewidth=1.1, edgecolor=COLOR_BORDER_STRONG, facecolor=COLOR_BG
    )
    top_cap = Ellipse(
        (x + w / 2, y + h - cap_h / 2), w, cap_h, linewidth=1.1, edgecolor=COLOR_BORDER_STRONG, facecolor=COLOR_BG
    )
    ax.add_patch(bottom_cap)
    ax.add_patch(top_cap)

    ax.text(x + w / 2, y + h * 0.42, label, fontsize=9.5, color=COLOR_TEXT, ha="center", va="center", **FONT_TITLE)
    if engine:
        ax.text(
            x + w / 2, y + h * 0.20, engine, fontsize=7.5, color=COLOR_TEXT_MUTED, ha="center", va="center", **FONT_BODY
        )
...
def draw_queue(ax, x, y, w, h, label, sublabel=None):
    ...
    text_x = x + w * 0.30
    if sublabel:
        ax.text(text_x, y + h * 0.60, label, fontsize=9, color=COLOR_TEXT, ha="left", va="center", **FONT_TITLE)
        ax.text(
            text_x, y + h * 0.32, sublabel, fontsize=7.5, color=COLOR_TEXT_MUTED, ha="left", va="center", **FONT_BODY
        )
    else:
        ax.text(text_x, y + h / 2, label, fontsize=9, color=COLOR_TEXT, ha="left", va="center", **FONT_TITLE)
...
def draw_arrow(ax, p1, p2, label=None, color=COLOR_BORDER_STRONG, label_offset=0.14):
    ...
    arrow = FancyArrowPatch(
        p1, p2, arrowstyle="-|>", mutation_scale=11, linewidth=1.2, color=color, shrinkA=2, shrinkB=2
    )
    ax.add_patch(arrow)
    if label:
        mx, my = (p1[0] + p2[0]) / 2, (p1[1] + p2[1]) / 2
        ax.text(
            mx, my + label_offset, label, fontsize=7.5, color=COLOR_TEXT_MUTED, ha="center", va="bottom", **FONT_BODY
        )
...
def draw_dashed_arrow(ax, p1, p2, label=None, color=COLOR_FAIL, label_offset=0.14):
    ...
    if label:
        mx, my = (p1[0] + p2[0]) / 2, (p1[1] + p2[1]) / 2
        ax.text(mx, my + label_offset, label, fontsize=7.5, color=color, ha="center", va="bottom", **FONT_BODY)
...
def draw_badge(ax, x, y, text, fail=False):
    ...
    h = GRID * 0.7
    box = FancyBboxPatch(
        (x, y), w, h, boxstyle="round,pad=0,rounding_size=0.35", linewidth=0.9, edgecolor=color, facecolor=soft
    )
    ax.add_patch(box)
    ax.text(x + w / 2, y + h / 2, text, fontsize=7, color=color, ha="center", va="center", **FONT_TITLE)
    return w
...
def draw_legend(ax, x, y, items):
    ...
    for style, label in items:
        ...
        ax.text(
            cursor + GRID * 0.95, y, label, fontsize=7.5, color=COLOR_TEXT_MUTED, ha="left", va="center", **FONT_BODY
        )
