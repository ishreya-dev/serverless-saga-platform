...
from diagram_lib import (
    new_canvas, draw_title, draw_card, draw_database, draw_arrow,
    draw_section, draw_badge, save, COLOR_TEXT_MUTED,
)
...
def main():
    ...
    ax.text(pg_x, pg_y - 0.05, "UNIQUE(idempotency_key)\nFK(saga_id)", fontsize=7.5,
             color=COLOR_TEXT_MUTED, ha="left", va="top", family="DejaVu Sans")
    ...
    draw_section(ax, ddb_x - 0.3, ddb_anchors[-1]["bottom"][1] - 0.35,
                 3.5, top_y + 0.8 - (ddb_anchors[-1]["bottom"][1] - 0.3) + 0.35,
                 "Saga State (DynamoDB)")

    for i in range(len(ddb_anchors) - 1):
        draw_arrow(ax, ddb_anchors[i]["bottom"], ddb_anchors[i + 1]["top"],
                   label="same saga_id" if i == 0 else None)
    ...
    draw_arrow(
        ax, ledger["right"], ddb_anchors[0]["left"],
    )
